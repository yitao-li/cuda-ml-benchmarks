...
runners$run_variations(
  algos = list(algorithms$algorithm_by_name("UMAP-Supervised")),
  ...
  benchmark(
    {
      ...
      # https://github.com/rapidsai/cuml/blob/branch-21.10/python/cuml/benchmark/algorithms.py#L443
      # here to ensure benchmarks done using {cuda.ml} will be comparible to those
      # done using {reticulate}+cuML
      cuda_ml_umap(x = cuda_ml_input_x, y = cuda_ml_input_y, n_neighbors = 5L, n_epochs = 500L)
    },
    replications = 10
  )
)
...
# BEGIN: {cuda.ml} benchmark
#                                                                                                test
# 1 {\n    cuda_ml_umap(x = cuda_ml_input_x, y = cuda_ml_input_y, n_neighbors = 5, n_epochs = 500)\n}
#   replications elapsed relative user.self sys.self user.child sys.child
# 1           10   4.747        1     1.214    3.533          0         0
#
# END: {cuda.ml} benchmark
# ```
#
...
#   for {cuda.ml} was around 0.4747 s/run.
