...
dataset_name <- "blobs"
dataset_n_samples <- 5000L
dataset_n_features <- 5000L

cat("\nBEGIN: cuML vs. scikit-learn benchmark:\n")
runners$run_variations(
  algos = list(algorithms$algorithm_by_name("PCA")),
  ...
  benchmark(
    {
      ...
      # https://github.com/rapidsai/cuml/blob/branch-21.10/python/cuml/benchmark/algorithms.py#L215
      # here to ensure benchmarks done using {cuda.ml} will be comparible to those
      # done using {reticulate}+cuML
      cuda_ml_pca(x = cuda_ml_input, n_components = 10L)
    },
    replications = 10
  )
)
...
# 0  PCA  numpy  0.516633  14.584032       0.0      0.0  28.229022       5000        5000
#
# END: cuML vs. scikit-learn benchmark:
#
#
# BEGIN: {cuda.ml} benchmark
#                                                          test replications
# 1 {\n    cuda_ml_pca(x = cuda_ml_input, n_components = 10)\n}           10
#   elapsed relative user.self sys.self user.child sys.child
# 1   20.66        1    16.921    3.733          0         0
#
# END: {cuda.ml} benchmark
# ```
#
...
#   for {cuda.ml} was around 2.066 s/run.
