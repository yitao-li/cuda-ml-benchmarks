...
runners$run_variations(
  algos = list(algorithms$algorithm_by_name("RandomForestClassifier")),
...
cuda_ml_input_x <- force(data[[1]])
cuda_ml_input_y <- force(data[[2]])
  ...
  benchmark(
    {
      ...
      # https://github.com/rapidsai/cuml/blob/branch-21.10/python/cuml/benchmark/algorithms.py#L303
      # here to ensure benchmarks done using {cuda.ml} will be comparible to those
      # done using {reticulate}+cuML
      cuda_ml_rand_forest(x = cuda_ml_input_x, y = cuda_ml_input_y, max_predictors_per_note_split = dataset_n_features, trees = 10L)
    },
    replications = 10
  )
)
...
# 0  RandomForestClassifier  numpy  0.028174  0.713797  0.991899  0.991899  25.335167      10000          10
#
# END: cuML vs. scikit-learn benchmark:
#
#
# BEGIN: {cuda.ml} benchmark
#                                                                                                                                      test
# 1 {\n    cuda_ml_rand_forest(x = cuda_ml_input_x, y = cuda_ml_input_y, max_predictors_per_note_split = dataset_n_features, trees = 10)\n}
#   replications elapsed relative user.self sys.self user.child sys.child
# 1           10   0.339        1     0.738     0.07          0         0
#
# END: {cuda.ml} benchmark
# ```
#
...
#   per run for {cuda.ml} was around 0.039 s/run.
